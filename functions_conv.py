...
class Pooling2DGrad(Function):
    ...
    def forward(self, gy):
        ...
        gcol = xp.zeros((N * C * KH * KW * OH * OW), dtype=self.dtype)
...
# numpy im2col
# ====================================================================================
def im2col_array(img, kernel_size, stride=1, pad=0, to_matrix=True):
    N, C, H, W = img.shape
...
def col2im_array(col, img_shape, kernel_size, stride=1, pad=0, to_matrix=True):
    N, C, H, W = img_shape
    KH, KW = pair(kernel_size)
    SH, SW = pair(stride)
    PH, PW = pair(pad)
